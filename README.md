...
graph TD
    ...
    classDef processing fill:#334155,stroke:#64748b,color:#f8fafc
    classDef database fill:#020617,stroke:#10b981,stroke-width:2px,color:#f8fafc
    classDef queue fill:#475569,stroke:#f59e0b,color:#f8fafc

    subgraph Client Layer
        U1[Web Browser]:::client
        U2[Mobile App]:::client
        U3[Phone / Voice]:::client
    end

    subgraph Gateway / Real-Time Layer
        LB[Load Balancer<br>Nginx/Envoy]:::gateway
        FAPI[FastAPI Service Cluster<br>WebSocket Handlers | Auth Management]:::gateway
        REDIS[(Redis<br>Session & Semantic Cache)]:::database
        MEDIA[Media Server<br>LiveKit/WebRTC]:::gateway
    end

    %% Broker
    KAFKA{{Kafka Message Broker}}:::queue

    subgraph Processing Layer
        RAG[Chatbot & RAG Workers<br>Retrieval / Embedding / LLM]:::processing
        ASR[ASR Service<br>Speech-to-Text]:::processing
        TTS[TTS Service<br>Text-to-Speech]:::processing
        TICKETSVC[Ticketing Service]:::processing
        ANALYTICS[Analytics Service]:::processing
        
        INGEST[Data Ingestion Pipeline]:::processing
        DOC[Document Processing]:::processing
    end

    subgraph Databases
        VDB[(Knowledge Base<br>Vector DB/Qdrant)]:::database
        PG[(Relational DB<br>PostgreSQL)]:::database
        ES[(Ticket DB<br>Elasticsearch/OpenSearch)]:::database
    end
    ...
    FAPI -->|user.messages.raw| KAFKA
    KAFKA -->|agent.responses.generated| FAPI
    
    KAFKA -->|user.messages.raw| RAG
    RAG -->|agent.responses.generated| KAFKA
    
    MEDIA -->|audio streams| ASR
    ASR -->|asb.transcriptions| KAFKA
    KAFKA -->|tts.requests| TTS
    TTS -->|audio streams| MEDIA

    RAG <--> VDB
    INGEST --> DOC --> VDB

    KAFKA -->|ticket.events| TICKETSVC
    TICKETSVC --> PG
    TICKETSVC --> ES
    
    KAFKA -->|ticket.events| ANALYTICS
    TICKETSVC --> |Sync| ANALYTICS
```
